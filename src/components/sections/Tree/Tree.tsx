...
const l1_sym = ["Bored Bolls", "Premature shedding of bolls","Premature opening of bolls","Fed bolls but holes not visible","Spots on bolls","Colour of bolls changing","Delay in maturity","Retarded growth of bolls","Bolls failed to opened","Presence of larvae/ insects on bolls"]
const l2_sym = [[
...
function Tree() {
        ...
        cy.on('tap','node',function(evt){
            const node = evt.target._private.data.label;
            console.clear();
            // console.log(node); 
                ...
                if(l1_sym[l1] != node){
                    ...
                    switch(node+flag[count]){
                        case l1_sym[count]+"0":
                            ...
                            const layout = cy.layout({ name: 'breadthfirst' });
                            layout.run()
                            flag[count] = 1;
                            break;
            //                 case l1_sym[count]+"1":
